  ...
  try {
    ...
    if (accounts.length === 0) {
      ...
      await admin.from("engine_runs").insert({
        user_id: userId,
        status: "ok",
        message: "Missing accounts reminder written",
        meta_json: { wrote: ["engine_missing_accounts"] },
      });

      return NextResponse.json({ ok: true, wrote: ["engine_missing_accounts"] });
    }

    const totals = computeTotals(accounts, bills, income);

    const safeBody = [
      `Balance now: ${formatMoneyFromCents(totals.balance)}`,
      `Income (7d): ${formatMoneyFromCents(totals.income7Total)}`,
      `Bills (7d): ${formatMoneyFromCents(totals.bills7Total)}`,
      "",
      `Safe-to-spend (7d): ${formatMoneyFromCents(totals.safeToSpendWeek)}`,
      "",
      "Truth reminder:",
    ...
    const rows = [
      {
        ...
        body: safeBody,
        severity: severityForSafeToSpend(totals),
        status: "open",
        snoozed_until: null,
      ...
      },
      {
        ...
        body: buildUpcomingBillsBody(totals),
        severity: severityForUpcomingBills(totals),
        status: "open",
        snoozed_until: null,
      ...
      },
      {
        ...
        body: buildUpcomingIncomeBody(totals),
        severity: severityForUpcomingIncome(totals),
        status: "open",
        snoozed_until: null,
    ...
    // Close missing reminder if it exists and we now have accounts
    await admin
      .from("decision_inbox")
    ...
    await admin.from("engine_runs").insert({
      user_id: userId,
      status: "ok",
      message: "Engine run wrote core reminders",
      meta_json: { wrote: ["engine_safe_to_spend_week", "engine_upcoming_bills_14d", "engine_upcoming_income_14d"] },
    });

    return NextResponse.json({
      ok: true,
      wrote: ["engine_safe_to_spend_week", "engine_upcoming_bills_14d", "engine_upcoming_income_14d"],
    });
  } catch (e: any) {
